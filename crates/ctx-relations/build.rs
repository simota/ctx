// crates/ctx-relations/build.rs
//
// Phase 2 FFI shim: emit a C99 header (`include/ctx_relations.h`) describing
// the `pub extern "C"` surface in `src/ffi.rs` so downstream cgo
// integrations get a stable, version-controlled handshake.
//
// Mirrors the pioneer's pattern (crates/ctx-contract/build.rs, ctx-scan/build.rs).
...
fn main() {
    ...
    let out_path = PathBuf::from(out_dir).join("ctx_relations.h");
    ...
    match builder.generate() {
        Ok(bindings) => {
            ...
            let stable_path = stable_dir.join("ctx_relations.h");

#!/usr/bin/env bash
# verify.sh — DONE gate for the git-routes parity loop. Emits SUCCESS/FAILED + footer.
...
#   1. gate files (goal.md, verify.sh, run-loop.sh, tests/git_parity.rs) sha256-pinned
#      — mutation = REWARD_HACK/GOAL_DRIFT (AP-13/AP-16).
#   2. Go untouched (internal/**, cmd/**) vs origin/main.
#   3. go build ./... clean.
#   4. cli/web/symbols/mcp green + git_parity in-progress.
...
# --- Gate 4: the FOUR already-complete suites must stay FULLY GREEN ---
# (cli/web/symbols/mcp are done; the loop must not regress them — require ok.)
...
# --- ctx-web git_parity is the IN-PROGRESS suite: partial-green expected mid-loop.
# Do NOT require "ok"; count passing per-case tests. The DONE gate (run-loop.sh)
# requires the FULL count (BASE_GIT_TOTAL). Here we enforce only a non-regression
# FLOOR so an iteration can't make a previously-green git case red.
git_out=$(cargo test --manifest-path crates/ctx-web/Cargo.toml --test git_parity 2>&1)
echo "$git_out" | grep -qE "test result: (ok|FAILED)" || { echo "$git_out" | tail -20; fail "ctx-web git_parity suite did not run (compile/spawn error — not a real RED)"; }
...
if [ -f "$PINS/baseline_counts.env" ]; then
  ...
  [ "$GIT_PASS" -ge "${BASE_GIT:-0}" ] || fail "ctx-web git_parity count regressed below floor ($GIT_PASS < $BASE_GIT) — a green case went red"
fi
...
echo "VERIFY: SUCCESS — cli=$CLI_PASS web=$WEB_PASS sym=$SYM_PASS mcp=$MCP_PASS git=$GIT_PASS green; Go untouched; go build clean; no placeholders"
echo "NEXUS_LOOP_STATUS: CONTINUE"
echo "NEXUS_LOOP_SUMMARY: gates green (cli=$CLI_PASS web=$WEB_PASS sym=$SYM_PASS mcp=$MCP_PASS git=$GIT_PASS); count drives DONE"
exit 0

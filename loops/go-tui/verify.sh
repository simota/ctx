#!/usr/bin/env bash
# verify.sh — DONE gate for the Wave 4 tui-ratatui loop. Emits SUCCESS/FAILED + footer.
...
#   1. gate files (goal.md, verify.sh, run-loop.sh, tests/snapshot.rs + goldens) sha256-pinned
#      — mutation = REWARD_HACK/GOAL_DRIFT (AP-13/AP-16).
#   2. Go untouched (internal/**, cmd/**) vs origin/main.
#   3. go build ./... clean.
#   4. cli/web/symbols/mcp/git/cutover green + tui snapshot in-progress.
...
go build ./... >/tmp/go-build-tui.log 2>&1 || fail "go build ./... broken (see /tmp/go-build-tui.log)"

# --- Gate 4: the SIX already-complete suites must stay FULLY GREEN ---
# (cli/web/symbols/mcp/git_parity/cutover are done; the tui port must not regress them.)
...
CUT_PASS=$(run_green ctx-cli     crates/ctx-cli/Cargo.toml      cutover "")

# --- ctx-tui snapshot is the IN-PROGRESS suite: partial-green expected mid-loop.
# Do NOT require "ok"; count passing per-session tests. The DONE gate (run-loop.sh)
# requires the FULL count (BASE_TUI_TOTAL). Here we enforce only a non-regression
# FLOOR so an iteration can't make a previously-green session red.
tui_out=$(cargo test --manifest-path crates/ctx-tui/Cargo.toml --test snapshot 2>&1)
echo "$tui_out" | grep -qE "test result: (ok|FAILED)" || { echo "$tui_out" | tail -20; fail "ctx-tui snapshot suite did not run (compile/spawn error — not a real RED)"; }
...
if [ -f "$PINS/baseline_counts.env" ]; then
  ...
  [ "$CUT_PASS" -ge "${BASE_CUTOVER:-0}" ] || fail "ctx-cli cutover count regressed ($CUT_PASS < $BASE_CUTOVER)"
  [ "$TUI_PASS" -ge "${BASE_TUI:-0}" ] || fail "ctx-tui snapshot count regressed below floor ($TUI_PASS < $BASE_TUI) — a green session went red"
fi

# --- Gate 6: placeholder grep on changed Rust src — EXCLUDE ctx-tui and tests.
# ctx-tui is the in-progress crate built from a stub scaffold; mid-loop partial
# stubs are expected there and the snapshot oracle (empty frame != golden) is the
# real anti-stub gate. Gate 6 still guards the other (complete) crates.
changed_rs=$(git diff --name-only origin/main -- 'crates/**/*.rs' 2>/dev/null | grep -v '/tests/' | grep -v '^crates/ctx-tui/' || true)
if [ -n "$changed_rs" ]; then
  if git diff origin/main -- $changed_rs 2>/dev/null | grep -E '^\+' | grep -nE 'todo!\(|unimplemented!\(|unreachable!\("stub|// *STUB|TODO: *port' >/dev/null; then
    fail "placeholder/stub introduced in changed Rust src outside ctx-tui (AP-12)"
  fi
fi

echo "VERIFY: SUCCESS — cli=$CLI_PASS web=$WEB_PASS sym=$SYM_PASS mcp=$MCP_PASS git=$GIT_PASS cutover=$CUT_PASS tui=$TUI_PASS green; Go untouched; go build clean; no placeholders"
echo "NEXUS_LOOP_STATUS: CONTINUE"
echo "NEXUS_LOOP_SUMMARY: gates green (cli=$CLI_PASS web=$WEB_PASS sym=$SYM_PASS mcp=$MCP_PASS git=$GIT_PASS cutover=$CUT_PASS tui=$TUI_PASS); count drives DONE"
exit 0
